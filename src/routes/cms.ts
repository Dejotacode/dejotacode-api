...
const categoryExists = async (c: Context<AppEnv>, categoryId: number) => {
...
cms.get('/posts', async (c) => {
  const result = await c.env.DB.prepare(
    `SELECT id,title,slug,type,status,excerpt,content,
      category_id AS categoryId,updated_at AS updatedAt
     FROM posts
...
cms.put('/posts/:id', async (c) => {
  ...
  const current = await c.env.DB.prepare('SELECT id FROM posts WHERE id=? LIMIT 1').bind(id).first();
  if (!current) return fail(c, 'POST_NOT_FOUND', 'Conteúdo não encontrado.', 404);
  ...
  await c.env.DB.prepare(
    `UPDATE posts SET
      category_id=?,type=?,slug=?,title=?,excerpt=?,content=?,status=?,
